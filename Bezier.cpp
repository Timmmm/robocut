...
//
qreal curvinessness(const QPointF& p0, const QPointF& p1, const QPointF& p2, const QPointF& p3)
{
	auto c1 = -p0 + 3.0 * p1 - 3.0 * p2 + p3;
	auto c2 = p0 - 2.0 * p1 + p2;
	auto norm = 2.0 * QPointF::dotProduct(c1, c1) + QPointF::dotProduct(c1, c2) + QPointF::dotProduct(c2, c2);
	return std::sqrt(norm);
}

} // namespace

void addBezierToPolygon(
    QPolygonF& poly,
    const QPointF& p0,
    const QPointF& p1,
    const QPointF& p2,
