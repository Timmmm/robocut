...
// cutting paths.
double pathSortDistance(const QList<QPolygonF>& paths, QPointF startingPoint)
{
	double distance = 0.0;
...
QList<QPolygonF> sortPathsInsideFirst(const QList<QPolygonF>& paths, QPointF startingPoint)
{
	...
	std::sort(indices.begin(), indices.end(), [&](const IndexAndBB& a, const IndexAndBB& b) {
...
QList<QPolygonF> sortPathsGreedy(const QList<QPolygonF>& paths, QPointF startingPoint)
{
	...
	while (sorted.size() < paths.size())
	{
		...
		for (int i = pathOffset[closest.polygon]; i < pathOffset[closest.polygon] + paths[closest.polygon].size(); ++i)
		{
...
QList<QPolygonF> sortPaths(const QList<QPolygonF>& paths, PathSortMethod method, QPointF startingPoint)
{
	...
	case PathSortMethod::Best: {
		...
		auto bestIdx = std::distance(qualities.begin(), std::max_element(qualities.begin(), qualities.end()));
